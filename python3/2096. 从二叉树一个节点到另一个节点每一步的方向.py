...
class Solution:
    ...
    def getDirections(self, root: Optional[TreeNode], startValue: int, destValue: int) -> str:
        ...
        def dfs(node):
            ...
            if node.left:
                edges.append((node.val - 1, node.left.val - 1))
                d[(node.val - 1, node.left.val - 1)] = "L"
                dfs(node.left)

            if node.right:
                edges.append((node.val - 1, node.right.val - 1))
                d[(node.val - 1, node.right.val - 1)] = "R"
                dfs(node.right)

        dfs(root)

        lca = LCABinaryLifting(edges, root.val - 1)
        lca_node = lca.lca(startValue - 1, destValue - 1)

        ans1 = list()
        cur = startValue - 1
        while cur != lca_node:
            ans1.append("U")
            cur = lca.parent[cur][0]

        ans2 = list()
        cur = destValue - 1
        while cur != lca_node:
            ans2.append(d[(lca.parent[cur][0], cur)])
            cur = lca.parent[cur][0]

        return "".join(ans1 + ans2[::-1])
