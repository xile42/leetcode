...
#
#
#  Related Topics String Simulation 👍 617 👎 67
...
class Solution:
    ...
    def getLucky(self, s: str, k: int) -> int:
        ...
        map = {ascii_lowercase[idx]: idx+1 for idx in range(len(ascii_lowercase))}
        base_string = str()
        for char in s:
            base_string += str(map[char])

        result = 0
        for _ in range(k):
            result_this_round = 0
            for char in base_string:
                result_this_round += int(char)
            result = result_this_round
            base_string = str(result_this_round)
