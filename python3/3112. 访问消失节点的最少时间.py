...
class Dijkstra:
    ...
    def __init__(self, n: int, edges: List[List[int]], start: int, directed: bool = False, need_path: bool = False, disappear = None) -> None:
    ...
    def _run(self) -> None:
        ...
        while heap:
            ...
            for y, w in self.g[x]:
                new_dis_y = dis_x + w
                if new_dis_y < self.dis[y] and new_dis_y < self.disappear[y]:
...
class Solution:
    ...
    def minimumTime(self, n: int, edges: List[List[int]], disappear: List[int]) -> List[int]:
        ...
        dij = Dijkstra(n, edges, 0, disappear=disappear)
        ans = dij.get_dis()
        ans = [i if i != inf else -1 for i in ans]
