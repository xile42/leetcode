...
class Solution:
    ...
    def leftmostBuildingQueries(self, heights: List[int], queries: List[List[int]]) -> List[int]:
        ...
        n = len(heights)
        seg = SegTree(lambda x, y: x if x > y else y, -1, heights)
        ans = list()
        for i, j in queries:
            if i == j:
                ans.append(i)
            ...
            mx_height = heights[i]
            left = j
            k = seg.max_right(left, lambda x: x <= mx_height)
            # print("i: {}, j: {}, hi: {}, hj: {}, mx_height: {}, k: {}".format(i, j, heights[i], heights[j], mx_height, k))
            if k == n:
                ans.append(-1)
            else:
                ans.append(k)
