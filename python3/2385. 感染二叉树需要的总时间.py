...
class Solution:
    ...
    def amountOfTime(self, root: Optional[TreeNode], start: int) -> int:
        ...
        d = {node_val: new_node_val for new_node_val, node_val in enumerate(nodes)}
        edges = [[d[x], d[y], w] for x, y, w in edges]
        root_value = d[root_value]

        lca = LCABinaryLifting(edges, root_value)
        ans = 0
        for node in nodes:
            ans = max(ans, lca.dist_none_weighted(d[start], d[node]))
