...
class Solution:
    ...
    def employeeFreeTime(self, schedule: '[[Interval]]') -> '[Interval]':
        ...
        schedule = reduce(add, schedule)
        intervals = [[i.start, i.end - 1, 1] for i in schedule]

        xs, ns = DiffArray.diff_accumulate(intervals)
        xs, ns = xs[:-1], ns[:-1]

        ans = list()
        cur = 0
        for v, ite in groupby(ns):
            l = len(list(ite))
            if v != 0:
                cur += l
                continue
            ans.append(Interval(xs[cur], xs[cur + l]))
            cur += l
